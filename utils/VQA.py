import torch
# Lazy import modelscope to avoid dependency issues if not using MPLUG/OFA
# Modelscope will be imported only when MPLUG or OFA classes are instantiated
...
# Lazy import lavis - only needed for BLIP2
# lavis will be imported inside BLIP2 class if needed
from promptcap import PromptCap_VQA
import clip, sys

# Lazy import llava - only needed for LLaVA model
# llava imports will be done inside Llava class if needed
...
class MPLUG():
    def __init__(self, device, ckpt):
        try:
            ...
            raise ImportError("modelscope is required for MPLUG but not available. Please install it or use a different VQA model.") from e
...
class GIT:
    ...
    def process_image(self, image):
        # GIT processes images in vqa method, just return RGB image
...
class BLIP2:
    def __init__(self, device, ckpt='pretrain_flant5xl'):
        try:
            ...
            raise ImportError("lavis is required for BLIP2 but not available. Please install it or use a different VQA model.") from e
        self.device = device
...
class BLIP:
    ...
    def process_image(self, image):
        # BLIP processes images together with text in vqa method
        # Just return the image converted to RGB
        return image.convert('RGB')
...
class VILT:
    ...
    def process_image(self, image):
        # VILT processes images in vqa method, just return RGB image
        return image.convert('RGB')
...
class OFA:
    def __init__(self, device, ckpt='damo/ofa_visual-question-answering_pretrain_large_en'):
        try:
            ...
            raise ImportError("modelscope is required for OFA but not available. Please install it or use a different VQA model.") from e
    ...
    def vqa(self, **kwargs):
        ...
        return result[self.OutputKeys.TEXT][0]
...
class Llava():
    def __init__(
        ...
        # Store imports for use in vqa method
    ...
    def process_image(self, image):
        pixel_values = self.image_processor.preprocess(image, return_tensors='pt')['pixel_values'].half()
        if torch.cuda.is_available():
            return pixel_values.cuda()
        elif hasattr(torch.backends, 'mps') and torch.backends.mps.is_available():
            return pixel_values.to('mps')
        else:
            return pixel_values
    ...
    def vqa(self, **kwargs):
        ...
        input_ids = self.tokenizer_image_token(prompt, self.tokenizer, self.IMAGE_TOKEN_INDEX, return_tensors='pt').unsqueeze(0)
        if torch.cuda.is_available():
